...
open class LightboxController: UIViewController {
    ...
    open override func viewDidLayoutSubviews() {
        ...
        let height = view.bounds.height * 0.15 > 88 ? view.bounds.height * 0.15 : 88
            ...
            DispatchQueue.global().async {
                if let urlData = NSData(contentsOf: url) {
                    DispatchQueue.main.async {
                        ...
                        UISaveVideoAtPathToSavedPhotosAlbum(filePath, `self`, #selector(self.image(_:didFinishSavingWithError:contextInfo:)), nil)
                    ...
                    DispatchQueue.main.async {
                        self.downloadFailDelegate?.lightboxControllerDownloadFail(self, error: NSError(domain: "", code: NSURLErrorFileDoesNotExist, userInfo:nil))
                        headerView.hideActivityIndicator()
                    }
                }
            }
        } else if let previewImage = self.pageViews[self.currentPage].imageView.image {
